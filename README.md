...
- **Numerical equivalence** — generated kernels match a CPU reference *on real hardware* (validated on 9 targets: Apple Metal, Vulkan/MoltenVK, Huawei Ascend NPU, NVIDIA GPU, Google TPU, AWS Trainium, Intel Gaudi, AMD Ryzen AI (XDNA2 NPU), and the `linalg`→CPU bridge — five with benchmark entries at [pu-rs.org](https://pu-rs.org); every backend is also checked by a codegen generality test suite)
...
| AMD Ryzen AI    | `aie`     | IRON (Python)                | ✅ | ✅ Ryzen AI Max+ 395 (XDNA2) |
...
All 15 emit their target language (codegen-tested); **9 are validated end-to-end on real hardware** today — Apple Metal (M2 Max / M4), Vulkan/SPIR-V (MoltenVK on Apple Silicon), Huawei Ascend NPU (910B), NVIDIA GPU (Tesla T4 / H20, via `mlir_to_cuda`), Google TPU (v5e, via `mlir_to_tpu` Pallas), AWS Trainium (trn1, via `mlir_to_nki`), Intel Gaudi (cloud, via `mlir_to_gaudi`), AMD Ryzen AI (XDNA2 NPU on a Ryzen AI Max+ 395, via `mlir_to_aie`), and the portable `linalg` bridge (CPU). Five carry published [pu-rs.org](https://pu-rs.org) benchmark entries (Metal, Ascend, NVIDIA, TPU, Trainium); Vulkan, Gaudi, AMD Ryzen AI, and the CPU `linalg` path are validated by standalone runs.

The AMD path also exercises the framework's portability claim in an unusual way: the `cuda` backend's generated kernels run unmodified on an AMD iGPU (gfx1151) through a small header shim, so one lowering serves both vendors' GPUs while `aie` targets the same machine's NPU.
