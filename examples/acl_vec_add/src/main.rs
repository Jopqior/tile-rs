...
unsafe extern "C" {
    fn vec_add_do(dim: u32, stream: *mut c_void, x: *mut c_void, y: *mut c_void, z: *mut c_void);
}
...
fn main() -> Result<()> {
    ...
    let x_host = common::read_buf_from_file::<AclFloat16>("test_data/input_x.bin");
    info!("Read {} elems for vec X", x_host.len());

    info!("Attempting to read vec Y from 'test_data/input_y.bin'...");
    let y_host = common::read_buf_from_file::<AclFloat16>("test_data/input_y.bin");
    ...
    let mut z_device = unsafe {
        DeviceBuffer::<AclFloat16>::uninitialized_with_policy(
            x_host.len(),
    ...
    unsafe {
        ...
        vec_add_do(block_dim, stream.to_raw(), args[0], args[1], args[2]);
    }
