// =============================================================================
// Benchmark: Rust vectorized f16 vec_add kernel on Ascend NPU
// =============================================================================
//
// Measures device-side execution time using AclEvent for the Rust vector-
// intrinsic vec_add kernel across multiple input sizes. Outputs CSV results.
// Includes correctness verification against CPU reference.
//
// Usage:
//   bench_vec_add_rs [--csv <path>]
...
fn choose_block_dim(total_size: usize) -> u32 {
    ...
    // Ensure total_size is evenly divisible by block_dim
...
fn check_correctness(npu_output: &[AclFloat16], x: &[AclFloat16], y: &[AclFloat16]) -> (bool, f32) {
    ...
    // f16 has limited precision (~1e-3), allow 1e-2 tolerance
...
fn bench_size(
    ...
    let mut z_device = unsafe {
        DeviceBuffer::<AclFloat16>::uninitialized_with_policy(size, AclrtMemMallocPolicy::HugeFirst)
            .context("z_device alloc")?
    };

    let kernel = kernel_loader
        .get_kernel("vec_add")
        .context("get_kernel(vec_add)")?;
    ...
    for _ in 0..WARMUP_ITERS {
        unsafe {
            let mut args: [*mut c_void; 4] = [
                ...
                len_device.as_mut_ptr() as *mut _,
            ];
    ...
    info!(
        "  rust_vector size={} block_dim={}: times={:?}",
...
fn main() -> Result<()> {
    ...
    for &size in SIZES {
        info!("Benchmarking rust_vector vec_add, size={}", size);
        let result = bench_size(&kernel_loader, &stream, size)?;

        let status = if result.pass { "PASS" } else { "FAIL" };
        eprintln!(
            "[correctness] rust_vector size={}: {} (max_err={:.2e})",
        ...
        for (run, &t) in result.times.iter().enumerate() {
            let line = format!("BENCH,{},rust_vector,{},{:.6}", size, run, t);
            println!("{}", line);
            csv_lines.push(line);
        }
    }
