// =============================================================================
// C++ matmul benchmark kernel (f16 x f16 -> f32, fixed 32x32x32)
// =============================================================================
// Based on CANN sample matmul_custom.cpp with host wrapper for Rust harness.
...
extern "C" __global__ __aicore__ void matmul_bench(GM_ADDR a, GM_ADDR b, GM_ADDR c)
{
    KernelMatmul op;
    op.Init(a, b, c);
    op.Process();
...
extern "C" void matmul_bench_do(uint32_t blockDim, void *stream, uint8_t *a, uint8_t *b, uint8_t *c)
{
    matmul_bench<<<blockDim, nullptr, stream>>>(a, b, c);
}
