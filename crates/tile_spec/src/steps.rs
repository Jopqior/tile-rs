//! Step definitions for the tile-rs `.feature` files.
//!
//! Shared by this crate's own harness (`tests/cucumber.rs`) and by any crate
//! that registers further targets and wants to run the same scenarios against
//! them: pass a registry factory to [`build_runner`].
...
/// Emit `mlir` for the backend registered as `target`. Returns the emitted
/// source or the emitter's error string. Goes through the same registry a
/// user's `TILERS_CODEGEN_PATH` selection does.
pub fn emit(make_registry: fn() -> TargetRegistry, target: &str, mlir: &str) -> Result<String, String> {
    let reg = make_registry();
    let t = reg
        .select(target)
        .ok_or_else(|| format!("unknown target '{target}' (registered: {:?})", reg.names()))?;
    t.emit(mlir, &EmitOpts::default()).map(|out| out.source)
}
...
/// Build the runner with every step definition wired to real code. Emit steps
/// select their target from the registry `make_registry` returns.
pub fn build_runner(make_registry: fn() -> TargetRegistry) -> Runner {
    ...
    r.when("I emit the snippet to target \"{}\"", move |w, a| {
        let target = &a[0];
        w.set("target", target);
        match emit(make_registry, target, w.get("mlir")) {
            Ok(src) => {
                w.set("emitted", src);
    ...
    r.when("I emit an empty module to target \"{}\"", move |w, a| {
        let target = &a[0];
        w.set("target", target);
        w.set_flag("emit_ok", emit(make_registry, target, "   \n  ").is_ok());
    });
    ...
    r.then(
        ...
        move |w, _| {
            let again = emit(make_registry, w.get("target"), w.get("mlir")).expect("re-emit ok");
            assert_eq!(
                again,
                w.get("emitted"),
...
/// Run every `.feature` file in `dir`, returning (files, scenarios).
pub fn run_feature_dir(runner: &Runner, dir: &Path) -> (usize, usize) {
