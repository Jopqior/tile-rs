...
// Async pipeline softmax — Future-based DMA with block_on executor.
// Zero manual pipe_barrier() calls. Barriers are inserted by Future::poll.
...
pub fn softmax_async(input: *const f32, output: *mut f32, len_buf: *const u32) {
    unsafe {
        ...
        pipeline::block_on(pipeline::store_f32_async(output, out, n));
    }
}
