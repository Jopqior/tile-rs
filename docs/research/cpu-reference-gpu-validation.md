...
5. **判据按算子族给、按各自自由度推**：单次四则在 MSL 中是正确舍入；`exp` 规范给的是 ulp 上界
   （关闭 fast math 时 `<= 4 ulp`）；`a*b+c` 默认允许被收缩成 FMA；`(a+b)*c` 受影响的是重结合而非收缩；
...
一例）。GPU 上的参考可能与被测 kernel 共享 GPU 编译器/驱动/库；若共同缺陷以相同方式影响两条路径，比较可能漏检。
独立 CPU 实现通常减少这些 GPU 层面的共享，但也不能保证参考无错。**这是相对程度，不是“GPU 参考不能验证数值正确性”的结论，第一方也不要求必须有 GPU
...
跨设备比较需显式关闭（§3.1）；④ 已选定按计算族给界，具体界的依据与取值尚待确定（§3.4）；
